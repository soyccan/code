...
public:
    ...
    int _gauss() {
        ...
        FOR(i, 0, _rows) {
            for (size_t j = i+1; j < _rows && sgn(_arr[i][i]) == 0; ++j) {
...
template<typename T>
Matrix<T> augment(Matrix<T>& x, Matrix<T>& y) {
...
template<typename T>
Matrix<T> mul(Matrix<T>& x, Matrix<T>& y) {
    _check_mul(x, y);
...
template<typename T>
SquareMatrix<T> identity_matrix(size_t size) {
...
template<typename T>
SquareMatrix<T> inverse(SquareMatrix<T>& matrix) {
